...
layout(std140, set = 1, binding = 0) uniform m_CircularProgressBarParameters
{
	vec2 m_CenterPos;
	float m_OuterRadius;
	float m_InnerRadius;
	float m_OuterGlowRadius;
	float m_InnerGlowRadius;

	float m_Alpha;

	vec4 m_RingForegroundColour;
	vec4 m_RingBackgroundColour;
	vec4 m_GlowColour; // Normalized
